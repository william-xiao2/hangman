...
class Hangman(object):
    ...
    def _word_guessed(self):
        """
        Returns: True if the word has completely been uncovered, False otherwise.
        """
        return not '_' in self._hidden_word

    def _game_done(self):
        """
        Returns: True if the hangman game has finished, False otherwise.

        The game has finished if the player has no lives or has
        guessed the entire word.
        """
        return self._guesses == 0 or self._word_guessed()
